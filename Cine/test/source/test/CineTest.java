...
/**
 * Esta es la clase usada para verificar que los métodos de la clase Cine estén correctamente implementados
 */
public class CineTest extends TestCase
{
    ...
    public void testCrearTarjeta( )
    {
        ...
        try
        {
            assertEquals( "La tarjeta tiene un valor errado", Tarjeta.CARGA_INICIAL, cine.darSaldoTarjeta( 25 ) );
            assertEquals( "El cine debe tener el saldo de la tarjeta que vendió", Tarjeta.CARGA_INICIAL, cine.darTotalDinero( ) );
            try
            {
                cine.crearTarjeta( 25 );
    ...
    public void testPagoReservaEfectivo( )
    {
        ...
        try
        {
            ...
            assertEquals( "La silla no está vendida.", true, silla.estaVendida( ) );
            assertEquals( "El cine debe tener el saldo de la reserva que vendió", reserva.darSaldoReserva( ), cine.darTotalDinero( ) );
        }
        catch( Exception e )
        {
    ...
    public void testPagoReservaTarjeta( )
    {
        ...
        try
        {
            ...
            int nuevoSaldo = ( int ) ( Tarjeta.CARGA_INICIAL - ( reserva.darSaldoReserva( ) * ( 1 - Tarjeta.DESCUENTO ) ) );
            assertEquals( "La silla no está vendida.", true, silla.estaVendida( ) );
            assertEquals( "El saldo de la tarjeta es inválido", nuevoSaldo, cine.darSaldoTarjeta( 25 ) );
        }
        catch( Exception e )
        {
            fail( "Error al agregar la silla a la reserva" );
    ...
    public void testGuardarReserva( )
    {
        ...
        try
        {
            ...
            assertEquals( "La cédula de la reserva es invalida", 25, reserva.darCedula( ) );
            assertEquals( "La reserva no quedó guardada", reserva, cine.darReserva( 25 ) );
            assertEquals( "La reserva no reporta estar guardada", true, cine.estaGuardada( reserva ) );
        }
        catch( Exception e )
        {
            fail( "Error al agregar la silla a la reserva" );
    ...
    {
        setupEscenario2( );
        int totalSillas = ( Cine.FILAS_GENERAL + Cine.FILAS_PREFERENCIAL ) * Cine.SILLAS_POR_FILA;
        assertEquals( "La cantidad de sillas no es correcta", totalSillas, cine.darSillas( ).length );
        ...
        try
        {
            ...
            assertEquals( "La silla debe ser de General", true, silla.esGeneral( ) );
            assertEquals( "La silla debe costar como en General", 8000, silla.darCosto( ) );

            silla = cine.darSilla( filaPreferecial, 1 );
            assertEquals( "La silla debe ser de Preferencial", true, silla.esPreferencial( ) );
            assertEquals( "La silla debe costar como en Preferencial", 11000, silla.darCosto( ) );
        }
        catch( Exception e )
        {
